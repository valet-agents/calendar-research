...
- **heartbeat** (heartbeat): Fires once a day to scan the calendar for meetings starting 30–40 minutes from now. Declared inline in `valet.yaml`, so it's created automatically by the dashboard setup flow.
...
- **Heartbeat cadence**: edit the `every` value on the `heartbeat` channel in `valet.yaml`, then redeploy. The default `24h` runs once a day — drop it to `1h` or `5m` for partners with back-to-back schedules who want briefs landing close to the meeting time (more calendar-API traffic).
